...
- Migrated both remaining authority consumers to the shared verifier (DEC-016, tasks 6.3). Hardware-in-the-loop absence authority now resolves through `verify_authority` by exact equality on the subject `hardware-in-loop:<runner>` — replacing the substring match over whole record rows that PEER-REVIEW-3 exploited — and its private `robotics.hardware_in_loop.decision_log_path`/`decision_id_pattern` configuration keys were removed in favour of the shared frozen `decision_log` grammar. A ledger that exists but cannot be read, decoded, or trusted as an in-repository regular file now blocks the gate as a named evidence failure instead of masquerading as an unauthorised absence. Release aggregation converts a declared skip to a pass only for verifier-minted `GateResult.declared_skip_authority` whose subject names the producing gate; a bare `decision_id` string — previously trusted if non-empty — now fails the release verdict, as does authority borrowed from another gate's subject or a gate name containing the subject separator. `run_gate` rebuilds results structurally via `dataclasses.replace`, so future `GateResult` fields cannot be silently dropped by a hand-maintained field list.
