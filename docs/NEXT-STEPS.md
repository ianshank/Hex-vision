...
| 3 | Conformance / probe outcome semantics | Distinguish a probe that could not run from a probe that ran and missed. The conformance gate treats any non-detecting outcome as a contract violation, so when INV-1's declared mechanism blocked on an absent scanner the gate reported `pack jetson violates contract`. That is the exact `BLOCKED` versus `FAILED` collapse the harness exists to prevent, reappearing inside the gate that certifies the contract. Provisioning the scanner in CI removed the symptom; the gate should surface a blocked probe as blocked. | S (1 PR, 4-6 h) | A probe whose mechanism is unavailable yields `blocked` with exit 2 and names the missing tool, while a genuinely undetected violation still fails with exit 1. |
| 4 | M5 / planning truth | Reconcile OpenSpec checkboxes and `planning/roadmap_data.py` against collected traceability evidence; regenerate projections in the same change. | S (1 PR, 4-6 h) | `make projections` and `make traceability` green with no completed work left marked open. |
| 5 | Post-M5 hardening | Finish the Ruff `ANN`, `PT`, `TID` and stricter MyPy `Any` rollout at configuration and gate boundaries. | M (1-2 PRs, 8-16 h) | Each rule family enabled with typed fixtures and no blanket exclusions. |
| 6 | M5 / container proof | Build and scan the image on a daemon-backed runner for `amd64` and `arm64`. The sandbox that produced this work has no container runtime, so the Dockerfile is validated structurally only. | S (1 PR, 4-6 h) | Both images build in CI and pass the secret and vulnerability scans. |
